...
pub const Vec3 = struct {
    ...
    pub fn mulEq(self: *Vec3, scalar: f64) *Vec3 {
        self.data *= @splat(scalar);
        return self;
    }

    pub fn divEq(self: *Vec3, scalar: f64) *Vec3 {
        self.data /= @splat(scalar);
        return self;
    }
    ...
    pub fn div_scalar(u: *const Vec3, v: f64) Vec3 {
        return Vec3{ .data = u.data / @as(@Vector(3, f64), @splat(v)) };
    }

    pub fn dot(u: *const Vec3, v: Vec3) f64 {
        var res: f64 = @reduce(.Add, u.data * v.data);
        return res;
    }

    pub fn cross(u: *const Vec3, v: Vec3) Vec3 {
        var xd: f64 = u.data[1] * v.data[2] - u.data[2] * v.data[1];
        var yd: f64 = u.data[2] * v.data[0] - u.data[0] * v.data[2];
        var zd: f64 = u.data[0] * v.data[1] - u.data[1] * v.data[0];
    ...
    pub fn random(random_engine: std.rand.Random) Vec3 {
        return Vec3.init(random_engine.float(f64), random_engine.float(f64), random_engine.float(f64));
    }

    pub fn random_interval(random_engine: std.rand.Random, interval: Interval) Vec3 {
        var vec = Vec3.zero();
        vec.data[0] = interval.min + (interval.max - interval.min) * random_engine.float(f64);
        vec.data[1] = interval.min + (interval.max - interval.min) * random_engine.float(f64);
        vec.data[2] = interval.min + (interval.max - interval.min) * random_engine.float(f64);
    ...
    pub fn near_zero(v: *const Vec3) bool {
        ...
        return (std.math.fabs(v.data[0]) < s) and (std.math.fabs(v.data[1]) < s) and (std.math.fabs(v.data[2]) < s);
    }
    ...
    pub fn refract(uv: *const Vec3, n: Vec3, etai_over_etat: f64) Vec3 {
        var cos_theta = @min(uv.negate().dot(n), 1.0);

        var r_out_perp = (uv.add(n.mul_scalar(cos_theta))).mul_scalar(etai_over_etat);
        var r_out_parallel = n.mul_scalar(-std.math.sqrt(std.math.fabs(1.0 - r_out_perp.length_squared())));
    ...
    pub fn random_in_unit_disk(random_engine: std.rand.Random) Vec3 {
        while (true) {
            var p = Vec3.zero();
            p.data[0] = -1 + 2.0 * random_engine.float(f64);
            p.data[1] = -1 + 2.0 * random_engine.float(f64);
