...
pub const LambertianMaterial = struct {
    ...
    pub fn scatter(ctx: *anyopaque, random_engine: std.rand.Random, ray_in: Ray, record: HitRecord, attenuation: *Color3, ray_out: *Ray) bool {
        ...
        var self: *LambertianMaterial = @ptrCast(@alignCast(ctx));
...
pub const MetalMaterial = struct {
    ...
    pub fn init(col: Color3, f: f64) MetalMaterial {
    ...
    pub fn scatter(ctx: *anyopaque, random_engine: std.rand.Random, ray_in: Ray, record: HitRecord, attenuation: *Color3, ray_out: *Ray) bool {
        var self: *MetalMaterial = @ptrCast(@alignCast(ctx));
...
pub const DialectricMaterial = struct {
    ...
    pub fn init(refraction: f64) DialectricMaterial {
    ...
    pub fn scatter(ctx: *anyopaque, random_engine: std.rand.Random, ray_in: Ray, record: HitRecord, attenuation: *Color3, scattered: *Ray) bool {
        var self: *DialectricMaterial = @ptrCast(@alignCast(ctx));

        attenuation.* = Color3.init(1.0, 1.0, 1.0);
        var refraction_ratio = if (record.front_face) 1.0 / self.ir else self.ir;

        var unit_direciton = ray_in.direction.unit_vector();

        var cos_theta = @min(unit_direciton.negate().dot(record.normal), 1.0);
        var sin_theta = std.math.sqrt(1.0 - cos_theta * cos_theta);

        var cannot_refract = refraction_ratio * sin_theta > 1.0;
        var direction: Vec3 = undefined;

        if (cannot_refract or reflectance(cos_theta, refraction_ratio) > random_engine.float(f64)) {
